...
# Seleciona o vetor de 21 dias (T1 a T21)
vetor_21dias = [f"T{i}" for i in range(1, 22)]  # ['T1', ..., 'T21']

# Filtra datasets
X_train = df[df['Country_code'].isin(codigos_treino)][vetor_21dias].copy()
y_train = df[df['Country_code'].isin(codigos_treino)]['Wave'].copy()

X_test = df[df['Country_code'].isin(codigos_teste)][vetor_21dias].copy()
...
for i, v in enumerate(importancias):
    print(f'Feature: {vetor_21dias[i]}, Score: {v:.5f}')

# Plot
plt.bar(vetor_21dias, importancias)
plt.title("Importância das Features (Vetor 21 Dias)")
plt.show()
